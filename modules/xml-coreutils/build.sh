...
RELEASE=${RELEASE:="9.1"}
PACKNAME=${PACKNAME:="xml-coreutils"}
...
mkdir SOURCES
mkdir pkgs
wget http://prdownloads.sourceforge.net/xml-coreutils/xml-coreutils-${VERSION}.tar.gz -O SOURCES/${PACKNAME}-${VERSION}.tar.gz
ret=$?
if [ $ret -ne 0 ]; then
        echo "Error in getting xml-coreutils-${VERSION}.tar.gz... exiting"
        exit 1
fi
